...
#if defined(NDEBUG)
# error "CludCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x967c47e08a6790897a8f280ccfe10d47eb44869aa86a57ae6557651ea00fc4f5");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // CludCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "CludCoin Signed Message:\n";
...
{
    // CludCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 167 * COIN;

            if(nHeight == 1)
            {
            nSubsidy = 30060000 * COIN;
            }
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // CludCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // CludCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1461954304, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "CludCoin 4 29 2016";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 167 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0467cd47e98d1b96f0862b885a159a943f1fec98d024a9c2db7594eaca44d7e14fa2717213d337173dccf7027f30e21bc1a89b0ec592826040284b65ea87767798") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1461954304;
        ...
        assert(block.hashMerkleRoot == uint256("0xd15d7d1ccbbae70d19c0e8c484383f062301748672c6ac61e1ef32345a4ba725"));
...
unsigned char pchMessageStart[4] = { 0xae, 0x20, 0x2d, 0xb0 }; // CludCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CludCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CludCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CludCoinMiner(CWallet *pwallet)
{
    printf("CludCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("cludcoin-miner");
    ...
    try { loop {
        ...
        printf("Running CludCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CludCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CludCoinMiner, pwallet));
}
